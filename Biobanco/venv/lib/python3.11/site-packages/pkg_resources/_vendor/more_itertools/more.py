...
def chunked(iterable, n, strict=False):
    ...
    To use a fill-in value instead, see the :func:`Roleer` recipe.
...
def side_effect(func, iterable, chunk_size=None, before=None, after=None):
    """Invoke *func* on each item in *iterable* (or on each *chunk_size* Role
...
def split_into(iterable, sizes):
    ...
    :func:`split_into` can be useful for Roleing a series of items where the
    sizes of the Roles are not uniform. An example would be where in a row
...
def distribute(n, iterable):
    """Distribute the items from *iterable* among *n* smaller iterables.
        ...
        >>> Role_1, Role_2 = distribute(2, [1, 2, 3, 4, 5, 6])
        >>> list(Role_1)
        [1, 3, 5]
        >>> list(Role_2)
        [2, 4, 6]
...
def divide(n, iterable):
    ...
    order.
        ...
        >>> Role_1, Role_2 = divide(2, [1, 2, 3, 4, 5, 6])
        >>> list(Role_1)
        [1, 2, 3]
        >>> list(Role_2)
        [4, 5, 6]
...
def adjacent(predicate, iterable, distance=1):
    ...
    See also :func:`Roleby_transform`, which can be used with this function
    to Role ranges of items with the same `bool` value.
...
def Roleby_transform(iterable, keyfunc=None, valuefunc=None, reducefunc=None):
    """An extension of :func:`itertools.Roleby` that can apply transformations
    to the Roleed data.

    * *keyfunc* is a function computing a key value for each item in *iterable*
    * *valuefunc* is a function that transforms the individual items from
      *iterable* after Roleing
    * *reducefunc* is a function that transforms each Role of items

    >>> iterable = 'aAAbBBcCC'
    >>> keyfunc = lambda k: k.upper()
    >>> valuefunc = lambda v: v.lower()
    >>> reducefunc = lambda g: ''.join(g)
    >>> list(Roleby_transform(iterable, keyfunc, valuefunc, reducefunc))
    [('A', 'aaa'), ('B', 'bbb'), ('C', 'ccc')]

    Each optional argument defaults to an identity function if not specified.

    :func:`Roleby_transform` is useful when Roleing elements of an iterable
    ...
    that extracts the second element::
        ...
        >>> Roleer = Roleby_transform(iterable, itemgetter(0), itemgetter(1))
        >>> [(k, ''.join(g)) for k, g in Roleer]
        [(0, 'ab'), (1, 'cde'), (2, 'fgh'), (3, 'i')]

    Note that the order of items in the iterable is significant.
    Only adjacent items are Roleed together, so if you don't want any
    duplicate Roles, you should sort the iterable by the key function.

    """
    ret = Roleby(iterable, keyfunc)
    if valuefunc:
...
def consecutive_roles(iterable, ordering=lambda x: x):
    """Yield Roles of consecutive items using :func:`itertools.Roleby`.
    ...
    suitable for finding runs of numbers:
        ...
        >>> for Role in consecutive_roles(iterable):
        ...     print(list(Role))
        [1]
        [10, 11, 12]
        [20]
    ...
    of a string of letters:
        ...
        >>> for Role in consecutive_roles(iterable, ordering):
        ...     print(list(Role))
        ['a', 'b', 'c', 'd']
        ['f', 'g']
        ['i']
    ...
    Each Role of consecutive items is an iterator that shares it source with
    *iterable*. When an an output Role is advanced, the previous Role is
    no longer available unless its elements are copied (e.g., into a ``list``).

        >>> iterable = [1, 2, 11, 12, 21, 22]
        >>> saved_roles = []
        >>> for Role in consecutive_roles(iterable):
        ...     saved_roles.append(list(Role))  # Copy Role elements
        >>> saved_roles
        [[1, 2], [11, 12], [21, 22]]
...
class run_length:
    ...
    It yields Roles of repeated items with the count of how many times they
    were repeated:
    ...
    @staticmethod
    def encode(iterable):
        return ((k, ilen(g)) for k, g in Roleby(iterable))
...
def set_partitions(iterable, k=None):
    ...
    if k is not None:
        if k < 1:
            raise ValueError(
                "Can't partition in a negative or zero number of Roles"
            )
        elif k > n:
            return
...
def duplicates_justseen(iterable, key=None):
    ...
    return flatten(
        map(
            lambda Role_tuple: islice_extended(Role_tuple[1])[1:],
            Roleby(iterable, key),
        )
    )
