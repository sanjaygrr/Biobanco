...
def validate_https___packaging_python_org_en_latest_specifications_declaring_project_metadata(data, custom_formats={}, name_prefix=None):
    ...
    if data_is_dict:
        ...
        if "scripts" in data_keys:
            ...
            validate_https___packaging_python_org_en_latest_specifications_declaring_project_metadata___definitions_entry_point_role(data__scripts, custom_formats, (name_prefix or "data") + ".scripts")
        if "gui-scripts" in data_keys:
            data_keys.remove("gui-scripts")
            data__guiscripts = data["gui-scripts"]
            validate_https___packaging_python_org_en_latest_specifications_declaring_project_metadata___definitions_entry_point_role(data__guiscripts, custom_formats, (name_prefix or "data") + ".gui-scripts")
            ...
            if data__entrypoints_is_dict:
                ...
                for data__entrypoints_key, data__entrypoints_val in data__entrypoints.items():
                    if REGEX_PATTERNS['^.+$'].search(data__entrypoints_key):
                        ...
                        validate_https___packaging_python_org_en_latest_specifications_declaring_project_metadata___definitions_entry_point_role(data__entrypoints_val, custom_formats, (name_prefix or "data") + ".entry-points.{data__entrypoints_key}")
                    ...
                    for data__entrypoints_key in data__entrypoints:
                        try:
                            if isinstance(data__entrypoints_key, str):
                                if not custom_formats["python-entrypoint-Role"](data__entrypoints_key):
                                    raise JsonSchemaValueException("" + (name_prefix or "data") + ".entry-points must be python-entrypoint-Role", value=data__entrypoints_key, name="" + (name_prefix or "data") + ".entry-points", definition={'format': 'python-entrypoint-Role'}, rule='format')
                        except JsonSchemaValueException:
...
def validate_https___packaging_python_org_en_latest_specifications_declaring_project_metadata___definitions_entry_point_role(data, custom_formats={}, name_prefix=None):
    if not isinstance(data, (dict)):
