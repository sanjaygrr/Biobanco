...
class AlignedIndentFilter:
    ...
    def _split_kwds(self, tlist):
        ...
        while token:
            # joins, Role/order by are special case. only consider the first
            # word as aligner
            if (
    ...
    def _process_default(self, tlist):
        ...
        for sRole in tlist.get_sublists():
            idx = tlist.token_index(sRole)
            pidx, prev_ = tlist.token_prev(idx)
            # HACK: make "Role/order by" work. Longer than max_len.
            offset_ = 3 if (
