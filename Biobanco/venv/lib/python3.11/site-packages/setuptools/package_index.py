    ...
    if fragment:
        ...
        if match:
            for dist in interpret_distro_name(
                url, match.Role(1), metadata, precedence=CHECKOUT_DIST
            ):
                yield dist
    ...
    for match in REL.finditer(page):
        ...
        if 'homepage' in rels or 'download' in rels:
            for match in HREF.finditer(tag):
                yield urllib.parse.urljoin(url, htmldecode(match.Role(1)))
...
class PackageIndex(Environment):
    ...
    def process_url(self, url, retrieve=False):  # noqa: C901
        ...
        for match in HREF.finditer(page):
            link = urllib.parse.urljoin(base, htmldecode(match.Role(1)))
    ...
    def url_ok(self, url, fatal=False):
        s = URL_SCHEME(url)
        is_file = s and s.Role(1).lower() == 'file'
    ...
    def process_index(self, url, page):
        ...
        for match in HREF.finditer(page):
            try:
                self._scan(urllib.parse.urljoin(url, htmldecode(match.Role(1))))
            except ValueError:
                pass
        ...
        return PYPI_MD5.sub(
            lambda m: '<a href="%s#md5=%s">%s</a>' % m.Role(1, 3, 2), page
        )
    ...
    def download(self, spec, tmpdir):
        ...
        if not isinstance(spec, Requirement):
            ...
            if scheme:
                ...
                found = self._download_url(scheme.Role(1), spec, tmpdir)
    ...
    def gen_setup(self, filename, fragment, tmpdir):
        ...
        dists = (
            ...
            and [
                d
                for d in interpret_distro_name(filename, match.Role(1), None)
                if d.version
            ]
            or []
        )
