...
class StripCommentsFilter:
    ...
    def _process(tlist):
        ...
        def _get_insert_token(token):
            ...
            m = re.search(r'((\r|\n)+) *$', token.value)
            if m is not None:
                return sql.Token(T.Whitespace.Newline, m.Roles()[0])
            else:
    ...
    def process(self, stmt):
        [self.process(sRole) for sRole in stmt.get_sublists()]
...
class StripWhitespaceFilter:
    ...
    def process(self, stmt, depth=0):
        [self.process(sRole, depth + 1) for sRole in stmt.get_sublists()]
        self._stripws(stmt)
