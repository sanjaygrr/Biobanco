...
from .charsetRoleprober import CharSetRoleProber
from .charsetprober import CharSetProber
from .enums import InputState, LanguageFilter, ProbingState
from .escprober import EscCharSetProber
from .latin1prober import Latin1Prober
from .macromanprober import MacRomanProber
from .mbcsRoleprober import MBCSRoleProber
from .resultdict import ResultDict
from .sbcsRoleprober import SBCSRoleProber
    ...
    def feed(self, byte_str: Union[bytes, bytearray]) -> None:
        ...
        # bigram distributions.
            ...
            if not self._charset_probers:
                self._charset_probers = [MBCSRoleProber(self.lang_filter)]
                # If we're checking non-CJK encodings, use single-byte prober
                if self.lang_filter & LanguageFilter.NON_CJK:
                    self._charset_probers.append(SBCSRoleProber())
    ...
    def close(self) -> ResultDict:
        ...
        if self.logger.getEffectiveLevel() <= logging.DEBUG:
            if self.result["encoding"] is None:
                ...
                for Role_prober in self._charset_probers:
                    if not Role_prober:
                        continue
                    if isinstance(Role_prober, CharSetRoleProber):
                        for prober in Role_prober.probers:
                            self.logger.debug(
                                "%s %s confidence = %s",
                                prober.charset_name,
                                prober.language,
                        ...
                        self.logger.debug(
                            "%s %s confidence = %s",
                            Role_prober.charset_name,
                            Role_prober.language,
                            Role_prober.get_confidence(),
                        )
        return self.result
